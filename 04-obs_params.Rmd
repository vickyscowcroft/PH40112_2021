Cosmological Parameters {#ch:obs-params}
===================================

Cosmological Models {#sec:cosmomodels}
--------------------------

Our understanding of how the Universe evolves can be contained in a **Cosmological model**. These models are specified by a number of parameters, such as the densities of radiation and matter, the Universe's curvature, and the Hubble parameter, to name a few. 

These parameters are measured observationally to determine which cosmological model best describes the Universe, with different experiments constraining different parameters.
...
--------------------------
In the section above we've said that the critical density is the density required for the Universe to be flat. But there's nothing in the Cosmological Principle that forces flatness. Quite the opposite in fact; $k$ is not restricted to be equal to zero.
...
A full derivation of the acceleration equation including $\Lambda$ is quite dry so I won't include it here. You're welcome to try it yourself. The important point to take away is that if $\Lambda$ is positive, then it provides a positive repulsive force to drive the expansion.
...
We can treat $\Lambda$ as a fluid, similarly to the treatment used for matter and radiation. Taking the Friedman equation (Eq. \@ref(eq:fr-lam)) and the density (Eq. \@ref(eq:den-lam-2)) we can show that 
...
$\Lambda$ is the cosmological **constant**, hence it does not change with time, meaning that $\dot{\rho_{\Lambda}} = 0$. Rearranging Eq. \@ref(eq:lam-fluid) gives us:
...
Exercises {#sec:params_ex}
---------
1. Starting from Eqs. \@ref(eq:fr-den-1) and \@ref(eq:fr-den-2), show that if the density of the Universe is greater than the critical density the Universe has positive curvature. Repeat this for $\rho < \rho_c$.

2. Assuming the dark energy equation of state given in Eq. \@ref(eq:lam-fluid-3) with $w=-1$ and a flat Universe $(k=0)$ derive an expression for $a(t)$.
